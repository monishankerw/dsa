...
//Convert a list of number to ASCII Character
        ...
        //All Repeated word
        Arrays.stream(str1.toLowerCase().split("\\s+")).collect(Collectors.groupingBy(Function.identity(), LinkedHashMap::new, Collectors.counting())).entrySet().stream().filter(e -> e.getValue() == 1).map(Map.Entry::getKey).findFirst();
        // Maximum occurence character
        input.chars().mapToObj(c -> (char) c).collect(Collectors.groupingBy(c -> c, Collectors.counting())).entrySet().stream().max(Map.Entry.comparingByValue()).map(Map.Entry::getKey).orElse(' ');
        // All Non Repeated Word
        Arrays.stream(str1.toLowerCase().split("\\s+")).collect(Collectors.groupingBy(Function.identity(), LinkedHashMap::new, Collectors.counting())).entrySet().stream().filter(e -> e.getValue() == 1).map(Map.Entry::getKey).forEach(System.out::println);
        //Freq Occ Character
        ...
        // Index of first Repeated char
        String strs = "swiss";
        String str11 = strs.toLowerCase();

        Optional<Character> firstRepeatedChar = str11.chars().mapToObj(c1 -> (char) c1).collect(Collectors.groupingBy(Function.identity(), LinkedHashMap::new, Collectors.counting())).entrySet().stream().filter(e -> e.getValue() > 1).map(Map.Entry::getKey).findFirst();

        if (firstRepeatedChar.isPresent()) {
            int index = str11.indexOf(firstRepeatedChar.get());
        ...
        String s1 = "listen";
        String s2 = "slient";
        boolean isAnagram = charFrequency(s1).equals(charFrequency(s2));
    ...
    private static Map<Character, Long> charFrequency(String str) {
